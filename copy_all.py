...
#set up temp directory for each processor with absolutely everything in it - 

def simulate(decision_vector):
    
    tmpdir = tempfile.mkdtemp(dir='/data/run/week_1_testing/PitzDailyOptimiser/')
    # print('PRINTING TEMPDIR', tmpdir)

    # if os.path.exists(tmpdir):
    #     shutil.rmtree(tmpdir)
    
    shutil.copytree('Exeter_CFD_Problems', tmpdir, dirs_exist_ok=True) #dirs_exist_ok=True
